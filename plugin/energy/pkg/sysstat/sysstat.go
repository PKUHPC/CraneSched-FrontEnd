...
func (s *SystemStats) GetCPUUtil(statData string) float64 {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SystemStats) GetDiskIO(diskData string) float64 {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SystemStats) GetNetworkIO(netData string) float64 {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SystemStats) GetCPUTemperature() (float64, error) {
...
func (s *SystemStats) GetCPUFrequency() (float64, error) {
	infos, err := cpu.Info()
	if err != nil {
		return 0, err
	}
