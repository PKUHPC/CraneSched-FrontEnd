...
func (c *PowerController) getNodesForWakeUpOrPowerOn(
	...
	requiredIdleNodeCount := int(float64(len(c.nodesInfo)) * c.config.Predictor.IdleReserveRatio)
	...
	log.Infof("[Time %v] Wake-up decision:\n"+
		...
		currentTime, currentActiveNodeCount, currentIdleNodeCount, requiredIdleNodeCount, totalAvailableNodeCount,
		requiredTotalNodeCount, predictedActiveNodeCount, neededNodeCount, len(nodesToWake), len(nodesToPowerOn))
...
func (c *PowerController) getNodesForSleepOrPowerOff(
	...
	if len(idleNodes) > 0 {
		requiredIdleCount := int(float64(len(c.nodesInfo)) * c.config.Predictor.IdleReserveRatio)
		currentIdleNodeCount := len(idleNodes)
	...
	log.Infof("[Time %v] Sleep/PowerOff decision:\n"+
		...
		"  Predicted active: %d\n"+
		"  Can sleep: %d\n"+
		"  Remaining idle after sleep: %d\n"+
		"  Nodes to sleep: %d\n"+
		"  Nodes to shutdown: %d\n",
		currentTime, currentActiveNodeCount, len(idleNodes),
		int(float64(len(c.nodesInfo))*c.config.Predictor.IdleReserveRatio),
		predictedActiveNodeCount,
		len(nodesToSleep), len(idleNodes)-len(nodesToSleep),
