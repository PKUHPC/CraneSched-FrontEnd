...
var (
	isSimulation         = true
	recordFilePath       = "/root/CraneSched-FrontEnd/plugin/powerControl/record.csv"
	nodeStateFilePath    = "/root/CraneSched-FrontEnd/plugin/powerControl/node_state_changes.csv"
	predictionRecordPath = "/root/CraneSched-FrontEnd/plugin/powerControl/predictions.csv"
)
...
func NewPowerController(
	...
	// 如果文件存在则删除
	files := []string{recordFilePath, nodeStateFilePath, predictionRecordPath}
	...
	if isSimulation {
		...
		for i := 0; i < 500; i++ {
			controller.InitializeNode(fmt.Sprintf("%d", i))
		}
...
// GetNodesJobCount 批量获取节点的作业数
func (c *PowerController) GetNodesJobCount(nodeIDs []string) map[string]int {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *PowerController) recordStateChange(time float64, nodeID string, oldState, newState NodeState) {
	if _, err := os.Stat(nodeStateFilePath); os.IsNotExist(err) {
		file, err := os.Create(nodeStateFilePath)
		if err != nil {
	...
	file, err := os.OpenFile(nodeStateFilePath, os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
...
func (c *PowerController) getPredictedActiveNodes() int {
	currentTime := c.time.Now()
	// 检查系统运行时间是否达到4小时
	if currentTime < 15000 { // 4小时 = 14400秒
		fmt.Printf("系统运行时间（%.2f秒）不足4小时，暂不执行预测\n", currentTime)
		return -1
	}

	reqBody := map[string]string{
		"csv_path": recordFilePath,
	}
	...
	// 将预测结果写入文件
	file, err := os.OpenFile(predictionRecordPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Warnf("Failed to open predictions file: %v", err)
	} else {
		defer file.Close()
		writer := csv.NewWriter(file)
		defer writer.Flush()
		...
		record := []string{
			...
			fmt.Sprintf("%d", predResp.Prediction),
		}

		if err := writer.Write(record); err != nil {
			log.Warnf("Failed to write prediction record: %v", err)
		}
	}
...
// 对外提供的接口
func (c *PowerController) RecordSystemState(currentTime float64, runningJobs int, waitingJobs int, currentPower float64) {
	...
	file, err := os.OpenFile(recordFilePath, os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
	...
	err = writer.Write([]string{
		...
		fmt.Sprintf("%d", runningJobs),
		fmt.Sprintf("%d", waitingJobs),
