...
class NodePredictor:
    ...
    def predict(self, avg_req_node_per_job, total_nodes):
        try:
            ...
            logging.info(f"Input parameters: avg_req_node_per_job={avg_req_node_per_job}, total_nodes={total_nodes}")
            ...
            logging.info(f"past_hour_data range: [{past_hour_data.min()}, {past_hour_data.max()}]")
            logging.info(f"past_hour_data samples:\n{past_hour_data[:6]}")
            
            logging.info(f"\ncur_datetime shape: {cur_datetime.shape}")
            logging.info(f"cur_datetime value: {cur_datetime}")
            
            logging.info(f"\ndayback_data shape: {dayback_data.shape}")
            logging.info(f"dayback_data value: {dayback_data}")
            ...
            with torch.no_grad():
                ...
                logging.info(f"past_hour_tensor range: [{past_hour_tensor.min().item()}, {past_hour_tensor.max().item()}]")
                
                logging.info(f"\ncur_datetime_tensor shape: {cur_datetime_tensor.shape}")
                logging.info(f"cur_datetime_tensor value: {cur_datetime_tensor}")
                
                logging.info(f"\ndayback_tensor shape: {dayback_tensor.shape}")
                logging.info(f"dayback_tensor value: {dayback_tensor}")
                ...
                logging.info(f"Scaled prediction shape: {prediction_scaled.shape}")
                logging.info(f"Scaled prediction value: {prediction_scaled.cpu().numpy()}")
                ...
                logging.info(f"\nInverse scaled prediction: {prediction}")
    ...
    def _get_dayback_features(self, df, current_idx, target_col):
        ...
        logging.info("\n=== Dayback Features Processing ===")
        logging.info(f"Current index: {current_idx}")
        ...
        for days_back in [1, 3, 5, 7]:
            ...
            logging.info(f"Raw value {days_back} days ago: {value}")
            dayback_features.append(value)

        dayback_features = np.array(dayback_features, dtype=np.float32)
        
        logging.info(f"Values before normalization: {dayback_features}")
        dayback_features = dayback_features
        logging.info(f"Values after normalization, before scaling: {dayback_features}")
        
        dayback_features = self.dayback_scaler.transform(
            dayback_features.reshape(1, -1)
        )
        
        logging.info(f"Final scaled values: {dayback_features[0]}")
