...
class NodePredictor:
    ...
    def _query_influx(self):
        ...
        try:
            ...
            logging.info(f"cluster_metrics: {cluster_metrics.head()}")
            logging.info(f"cluster_metrics: {cluster_metrics.tail()}")
    ...
    def predict(self, total_nodes):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                logging.info(f"After inverse transform: {prediction}")

                prediction_scaled_by_nodes = prediction * total_nodes
                logging.info(f"After scaling by total nodes ({total_nodes}): {prediction_scaled_by_nodes}")

                final_prediction = max(0, math.ceil(float(prediction_scaled_by_nodes[0][0])))
                logging.info(f"Final prediction (after ceiling): {final_prediction}")
...
def predict_nodes():
    ...
    try:
        ...
        logging.info(f"Received prediction request with total_nodes: {total_nodes}")
