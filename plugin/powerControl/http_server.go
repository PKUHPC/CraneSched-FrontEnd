...
func (s *HTTPServer) registerRoutes(router *mux.Router) {
	...
	// 添加获取节点作业数的接口
	router.HandleFunc("/nodes/{nodeID}/jobs/count", s.handleGetNodeJobCount).Methods("GET")

	// 修改为批量获取节点作业数的接口
	router.HandleFunc("/nodes/jobs/count", s.handleGetNodesJobCount).Methods("POST")
}
...
// 获取节点作业数的处理函数
func (s *HTTPServer) handleGetNodeJobCount(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	nodeID := vars["nodeID"]

	count := s.controller.GetNodeJobCount(nodeID)
	json.NewEncoder(w).Encode(map[string]interface{}{
		"count": count,
	})
}
...
// 批量获取节点作业数的处理函数
func (s *HTTPServer) handleGetNodesJobCount(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	counts := s.controller.GetNodesJobCount(req.NodeIDs)
	json.NewEncoder(w).Encode(map[string]interface{}{
		"counts": counts,
	})
}
