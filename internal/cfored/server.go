...
func (keeper *SupervisorChannelKeeper) setRemoteIoToCrunChannel(frontId int32, taskId uint32, stepId uint32, ioToCrunChannel chan *protos.StreamTaskIORequest) {
	...
	if keeper.taskIORequestChannelMap[StepIdentifier{taskId: taskId, StepId: stepId}] == nil {
		keeper.taskIORequestChannelMap[StepIdentifier{taskId: taskId, StepId: stepId}] = make(map[int32]chan *protos.StreamTaskIORequest)
	}
	keeper.taskIORequestChannelMap[StepIdentifier{taskId: taskId, StepId: stepId}][frontId] = ioToCrunChannel
	if keeper.taskIOBufferMap[StepIdentifier{taskId: taskId, StepId: stepId}] == nil {
		keeper.taskIOBufferMap[StepIdentifier{taskId: taskId, StepId: stepId}] = &TaskIOBuffer{
...
func (keeper *SupervisorChannelKeeper) forwardRemoteIoToCrun(taskId uint32, stepId uint32, ioToCrun *protos.StreamTaskIORequest) {
	keeper.taskIORequestChannelMtx.Lock()
	channelMap, exist := keeper.taskIORequestChannelMap[StepIdentifier{taskId: taskId, StepId: stepId}]
	if exist {
...
func (keeper *SupervisorChannelKeeper) crunTaskStopAndRemoveChannel(taskId uint32, stepId uint32) {
	...
	delete(keeper.taskIOBufferMap, StepIdentifier{taskId: taskId, StepId: stepId})
	keeper.taskIORequestChannelMtx.Unlock()
}
...
CforedSupervisorStateMachineLoop:
	for {
		...
		case SupervisorReg:
			...
			gSupervisorChanKeeper.supervisorUpAndSetMsgToSupervisorChannel(taskId, stepId, cranedId, pendingCrunReqToSupervisorChannel, pendingCattachReqToSupervisorChannel, valid)
		...
		supervisorIOForwarding:
			for {
				...
				case cattachReq := <-pendingCattachReqToSupervisorChannel:
					...
					default:
						log.Fatalf("[Cfored<->Supervisor][Step #%d.%d] Receive Unexpected %s from crun ",
							taskId, stepId, cattachReq.Type.String())
						break supervisorIOForwarding
					}
				}
			}
