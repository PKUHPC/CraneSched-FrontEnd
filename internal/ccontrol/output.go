...
func printResourceRequests(job *protos.JobInfo) {
	...
	fmt.Printf("\tReqRes:node=%d cpu=%.2f mem=%v gres=%s\n",
		...
		formatGresMap(job.ReqTotalResView.GresMap))
	// AllocRes
	if job.Status == protos.JobStatus_Running {
		fmt.Printf("\tAllocRes:node=%d cpu=%.2f mem=%v gres=%s\n",
			job.NodeNum,
			job.AllocatedResView.CpuCount,
			util.FormatMemToMB(job.AllocatedResView.MemoryBytes),
			formatGresMap(job.AllocatedResView.GresMap))
	}
}
