...
func ShowNodes(nodeName string, queryAll bool) {
	...
	if err != nil {
		util.GrpcErrorWithMsg(err, "QueryNodeInfo")
	}
...
func ShowPartitions(partitionName string, queryAll bool) {
	...
	if err != nil {
		util.GrpcErrorWithMsg(err, "QueryPartitionInfo")
	}
...
func ShowTasks(taskId uint32, queryAll bool) {
	...
	if err != nil {
		util.GrpcErrorWithMsg(err, "QueryTasksInfo")
	}
...
func ChangeTaskTimeLimit(taskId uint32, timeLimit string) {
	...
	if err != nil {
		util.GrpcErrorWithMsg(err, "ModifyTask")
	}
