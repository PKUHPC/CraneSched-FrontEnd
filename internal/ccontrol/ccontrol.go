 ...
 */
 ...
 func ShowConfig(ConfigPath string) {
	 ...
	 for _, line := range yamlData {
		 ...
		 if reflect.TypeOf(LineValue).Kind() != reflect.Slice {
			 fmt.Printf("%s = %v\n", LineKey, LineValue)
		 } else {
			 fmt.Printf("%s:\n", LineKey)
			 LineLength := reflect.ValueOf(LineValue).Len()
			 for ValueIndex := 0; ValueIndex < LineLength; ValueIndex++ {
				 Item := reflect.ValueOf(LineValue).Index(ValueIndex).Interface()
				 ItemLength := reflect.ValueOf(Item).Len()
				 for ItemIndex := 0; ItemIndex < ItemLength; ItemIndex++ {
					 ItemStruct := reflect.ValueOf(Item).Index(ItemIndex).Interface()
					 MapStruct, ok := ItemStruct.(yaml.MapItem)
					 if ok {
						 if ItemIndex == 0 {
							 fmt.Printf("\t- ")
						 } else {
