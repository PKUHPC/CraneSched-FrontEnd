...
func initPodFlags(cmd *cobra.Command) {
	cmd.Flags().BoolVar(&FlagContainer, "container", false, "Submit as container job (pod will be created)")
...
func overridePodFromFlags(cmd *cobra.Command, podOpts *podOptions) error {
	if cmd.Flags().Changed("container") {
		podOpts.container = FlagContainer
	}
	...
	if !podOpts.container && (podOpts.name != "" || len(podOpts.ports) > 0 || podOpts.user != "" || !podOpts.userns || podOpts.hostNet) {
		return fmt.Errorf("--container must be set when using pod-related flags")
	}
...
func buildPodMeta(task *protos.TaskToCtld, podOpts *podOptions) (*protos.PodTaskAdditionalMeta, error) {
	...
	for _, port := range podMeta.Ports {
		if port.HostPort < 1 || port.HostPort > 65535 || port.ContainerPort < 1 || port.ContainerPort > 65535 {
			return nil, fmt.Errorf("invalid port mapping: %d:%d", port.HostPort, port.ContainerPort)
		}
	}
