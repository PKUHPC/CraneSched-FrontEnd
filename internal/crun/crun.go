...
func MainCrun(args []string) util.CraneCmdError {
	...
	if FlagX11 {
		...
		if port == 0 {
			log.Debugf("Display is using UNIX socket: %s", target)
		} else {
			log.Debugf("Display is using TCP port: %d, hostname: %s", port, target)
		}

		cookie, err := util.X11GetXAuthCookie()
		if err != nil {
			log.Warning("GetX11AuthCookie Error: %s. X11 will be disabled.", err)
			interactiveMeta.X11 = false
		}
