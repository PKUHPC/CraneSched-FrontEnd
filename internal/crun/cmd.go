...
func init() {
	...
	RootCmd.Flags().Uint32VarP(&FlagNtasks, NtasksOptionStr, "n", 1, "Number of tasks to invoke for the job")
	RootCmd.Flags().Uint32Var(&FlagNtasksPerNode, NtasksPerNodeOptionStr, 0, "Maximum number of tasks to invoke on each node, 0 for unlimited")
