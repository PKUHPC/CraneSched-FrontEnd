...
func runExecute(cmd *cobra.Command, args []string) error {
	...
	if stepMode {
		jobId, err := strconv.ParseUint(envJobIdString, 10, 32)
		if err != nil {
			return util.NewCraneErr(util.ErrorCmdArg, fmt.Sprintf("invalid CRANE_JOB_ID '%s': %v", envJobIdString, err))
		}

		step, err := buildContainerStep(cmd, f, uint32(jobId), image, command)
		if err != nil {
		...
		reply, err := submitContainerStep(step)
		if f.Global.Json {
		...
		fmt.Printf("Container step submitted successfully. Job ID: %d, Step ID: %d\n", step.JobId, reply.GetStepId())
		return nil
	}

	// Build the container task
	task, err := buildContainerTask(cmd, f, image, command)
	if err != nil {
...
func buildContainerTask(cmd *cobra.Command, f *Flags, image string, command []string) (*protos.TaskToCtld, error) {
