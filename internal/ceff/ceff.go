...
func PrintJobInfo(jobInfo *protos.JobInfo, records []*ResourceUsageRecord) error {
	...
	cpuTotal := jobInfo.AllocatedResView.CpuCount
	...
	memEfficiency := 0.0
	mallocMemMbPerNode := float64(jobInfo.AllocatedResView.MemoryBytes) /
		float64(jobInfo.NodeNum) / (1024 * 1024)
	totalMallocMemMb := float64(jobInfo.ReqTotalResView.MemoryBytes)
